...
import com.sindhi.urdu.english.keybad.sindhikeyboard.jetpack_version.data.mySolidDarkThemes
...
@Composable
fun SolidDarkPage(myPreferences: MyPreferences, onApplyThemeClick: (CustomTheme) -> Unit, innerPadding: PaddingValues) {
    val selectedTheme = myPreferences.getTheme()
    val isSelectedLists = mySolidDarkThemes.mapIndexed { _, categoryThemes ->
        categoryThemes.mapIndexed { _, theme ->
            mutableStateOf(selectedTheme == theme.name)
        }
    }

    mySolidDarkThemes.forEachIndexed { categoryIndex, themes ->
