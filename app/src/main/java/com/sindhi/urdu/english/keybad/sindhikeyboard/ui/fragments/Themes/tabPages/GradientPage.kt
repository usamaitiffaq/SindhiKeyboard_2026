...
import com.sindhi.urdu.english.keybad.sindhikeyboard.jetpack_version.data.myGradientThemes
...
@Composable
fun GradientPage(myPreferences: MyPreferences, onApplyThemeClick: (CustomTheme) -> Unit, innerPadding: PaddingValues) {
    val selectedTheme = myPreferences.getTheme()
    val isSelectedLists = myGradientThemes.mapIndexed { _, categoryThemes ->
        categoryThemes.mapIndexed { _, theme ->
            mutableStateOf(selectedTheme == theme.name)
        }
    }

    myGradientThemes.forEachIndexed { categoryIndex, themes ->
