...
import com.sindhi.urdu.english.keybad.sindhikeyboard.jetpack_version.data.mySolidLightThemes
...
@Composable
fun SolidLightPage(myPreferences: MyPreferences, onApplyThemeClick: (CustomTheme) -> Unit, innerPadding: PaddingValues) {
    val selectedTheme = myPreferences.getTheme()
    val isSelectedLists = mySolidLightThemes.mapIndexed { _, categoryThemes ->
        categoryThemes.mapIndexed { _, theme ->
            mutableStateOf(selectedTheme == theme.name)
        }
    }

    mySolidLightThemes.forEachIndexed { categoryIndex, themes ->
